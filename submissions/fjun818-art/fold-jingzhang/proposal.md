...
本方案的总体概念为「**京张数智折叠带**」：一次「折叠」把 1909 年的铁路遗产与 2035 年的 AI 街区压入同一空间坐标——物理层保留铁路遗产肌理并植入模块化、可拆卸的「AI 城市插件」；数字层构建「可编辑城市底座」，把三片区串联为数据资产化试验带；体验层通过 AR/MR 在法定文保锚点上叠加百年铁路场景与未来 AI 愿景。「折叠」不是造型隐喻，而是数据要素在物理空间中流通、登记、交易、反馈的运行机制（详见「京张数据契约」专节）。
...
三大定位与五大功能在 JDSFB 中逐条落到机制与空间，形成功能统筹闭环 [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]：

| 官方要求 | JDSFB 机制应答 | 空间落点 |
| --- | --- | --- |
...
视频演示按「数据流转视频」与「AI 赋能视频」两类组织。成片规格统一为：MP4 封装（H.264 编码），分辨率不低于 1920×1080（1080p），帧率不低于 30fps，音频 AAC 编码（采样率 44.1kHz，比特率不低于 128kbps），单个视频文件不超过 200MB。每条视频配标准化文字说明（唯一标识符、精确时长、核心内容概述、关键时间节点标记），说明文字放置于视频播放器下方。AI 赋能视频 VID-AI-01 已交付并登记 manifest.json；数据流转视频素材制作中，到位后登记 manifest.json 并替换本节占位说明。

### 数据流转视频

- **标识符**：VID-DATA-01；**时长**：待剪辑后标注（分:秒）
- **核心内容概述**（不超过 150 字）：演示「感知—决策—执行—反馈」数据闭环——12 个数据节点感知采集，汇入 1+3+N 数字孪生底座，经京张数据契约三级清单授权与算法贡献指数计量，驱动 10 个 AI+ 运营场景执行，运行数据回流底座完成迭代。
- **关键时间节点标记**：00:00 片头与三层范围定位；数据流转主干演示；数据契约授权与合规审计；结尾（精确时点待剪辑后标注）。

### AI 赋能视频

- **标识符**：VID-AI-01；**时长**：00:10；**文件**：`assets/media/vid-ai-01-ai-empowerment.mp4`（MP4 · H.264 High · 1920×1080 · 30fps · AAC 44.1kHz 128kbps · 17.6MB）
...
- **关键时间节点标记**：00:00 片头「AI赋能 · 百年京张新范式」与发光轨道折叠意象；00:03 中缝折叠双时代对开（1909 蒸汽机车历史层 × 2035 高铁未来层 AR/MR 叠加）；00:07 AI 节点网络环绕数字孪生底座（AI+ 运营赋能）；00:10 结尾。
...
下表将 10 个运营环节落到「感知—决策—执行—反馈」可核查闭环：明确每个环节的感知数据来源、决策主体、执行载体与反馈通道，标注对应京张数据契约层级（L1/L2/L3，法源第 82 条）与落地图层，并给出运营考核指标。全部环节为运营建议性质，不构成政府投资、审批或收益承诺。
...
| AI+06 需求响应公交 | OD 需求·L2 | 微循环调度 | 廊道节点·[roads][phasing] | 上座率/候时反馈 | 法源第 52 条特色公交；考核：候时达标率、接驳衔接率 |
...
### 一致性验证：与国家城市数字更新行业指引的对齐结论

本节给出 JDSFB 场景设计与国家城市全域数字化转型、城市数字更新最新行业指引的**一致性验证结果**：经逐项比对，10 个 AI+ 运营环节在场景分类、总体原则、数字底座、场景结构与运营机制五个维度均与国家行业指引保持一致 [source:DIGITAL-RENEWAL-GUIDELINE]。

![AI+ 场景与国家城市数字更新行业指引一致性验证图](assets/figures/standards-alignment.png)

| 验证维度 | 一致性验证结论 |
| --- | --- |
| 场景分类 | 10 个 AI+ 环节全部可落入指引确立的城市数字更新场景体系（数字底座、综合街区、民生公共服务、历史文保、既有建筑、产城融合等类别），无游离于标准体系之外的自创场景 |
| 总体原则 | 环节设计贯彻指引倡导的总体原则：以人为本、安全可信、数据驱动、同步建设、集约利旧、多元参与、扩展升级、绿色低碳、系统协同 |
| 数字底座 | 1+3+N 数字孪生底座、边缘算力节点与京张数据契约，与指引对城市数字底座（基础设施、数据资源、智能中枢）的更新要求一致 |
| 场景结构 | 各环节按「目标—人群—活动—资源—环境—价值—成效—运营」结构组织，与指引推荐的场景组织方式一致 |
| 运营机制 | 各环节均明确运营闭环与可持续路径，且全部为运营建议性质，不构成政府投资、审批或收益承诺 |

**验证结论**：JDSFB 场景体系与国家城市数字更新行业指引方向一致、结构兼容、机制可对接，可作为指引在 AI 创新街区场景下的概念性落地参考。所有 AI 应用同时遵循本方案「伦理与数据治理」边界。
...
| JZ-06 | 7 折叠点 AR/MR 体验叠加工程、时空叙事内容共创平台 | 第 26 条文保清单 + 第 21—23 条重点地区 | P1 | 全国重点/市级文保仅体验叠加，物理改动 zero；实施前须文保审批 |
...
**分期实施时序**（与控规第 84—88 条实施策略衔接，阶段与 phasing.geojson 一致）：P1 近期试点聚焦轻量、可逆、低干预对象——JZ-01 慢行缝合、JZ-06 折叠点 AR/MR 体验叠加、JZ-08 活动周临时设施，验证「体验即运营」；P2 中期更新待正式控规图则、市政、交通与权属条件确认后推进 JZ-02—JZ-05 的空间改造与节点网络铺装；P3 长期治理以 JZ-07 数据契约运营平台为主，随数据要素制度成熟持续迭代。P1→P2→P3 递进依赖「条件成熟度」而非固定时长，避免在法定图则与权属未明确前提早启动不可逆工程。

**土地混合利用与功能转换（正面清单管理）**：面向近千万㎡存量空间与 100 万㎡ 更新载体，功能转换遵循国家城市更新政策「正面清单」原则——仅允许从低效工业、仓储、旧办公向科创、文创、公共服务、保障性住房等鼓励方向转换，禁止向高密住宅、超标高度等负面方向突破。混合利用以控规 75 个主导功能分区为底账，新功能不得与主导功能冲突，高度与强度严守 6 类基准高度分区与五级强度分区。
...
本节将 JDSFB 与《城市更新"十五五"规划》、国家持续推进城市更新行动相关文件及《城市更新规划编制导则》对齐，确保方案在指导思想、战略目标、重点任务与实施要求上与国家城市更新顶层设计一致 [source:CITY-RENEWAL-15TH-PLAN] [source:CITY-RENEWAL-ACTION-OPINION] [source:CITY-RENEWAL-PLANNING-GUIDE]。

### 重点领域对齐

| 国家规划重点领域 | 规划核心要求 | JDSFB 对齐设计 |
| --- | --- | --- |
| 城市空间布局优化 | 结构优化、功能完善、土地混合开发与用途合理转换、正面清单 | 「一带一轴两心多点」法定结构对位；75 个主导功能分区底账识别低效机遇空间；功能转换与混合利用按正面清单管理 |
...
按国家「专项规划—片区策划—项目实施方案」三级实施体系与「体检先行、无体检不更新」闭环，JDSFB 实施路径划分为四个阶段（均为概念建议的时间框架，具体以官方实施计划为准）：
...
- **政策支持**：依托国家城市更新用地政策（土地混合开发、用途依法合理转换正面清单、存量低效用地盘活、不超过 5 年过渡期政策）与房屋全生命周期安全管理制度；JDSFB 的可拆卸 AI 城市插件按临时设施属性管理，契合功能转换与混合利用导向。
- **资金保障**：参照国家多元化投融资框架——中央预算内投资与中央财政支持、地方政府专项债券（严禁违法违规举债）、金融机构信贷、基础设施 REITs 与资产证券化、政府与社会资本合作新机制、「谁受益谁出资」的政府/市场/居民合理共担机制。JDSFB 全部资金路径均为机制建议，不构成投资承诺或融资安排。
- **监督评估**：对齐国家「体检发现问题—更新解决问题—评估更新效果—推动巩固提升」闭环与规划实施的动态监测、中期评估、总结评估要求；JDSFB 以 CIM 数字底座支撑城市体检评估信息平台，算法贡献指数与场景使用频次作为运营期动态监测指标，项目库按「入库—出库—动态调整」管理。
...
以下给出核心指标的可执行复算口径、置信度与误差说明，以及在官方边界发布后的重算触发条件，确保「指标可核查、口径可追溯、边界可更新」。

| 指标 | 复算口径（EPSG:4548） | 置信度 | 误差来源 | 官方数据发布后重算触发 |
| --- | --- | --- | --- | --- |
| 总体设计范围面积 site_area_sqm | polygon_area(site_boundary.geojson) | 低 | 边界为公告口径临时示意几何，非控规法定红线 | 以官方边界重跑 `scripts/metrics.py` 面积随边界自动更新 |
...
| 总建筑规模上限 total_floor_area_ceiling_sqm | 读控规文本第 7/13 条常量 2408.0 万㎡ | 高（法定） | 无（法定约束，非测量量） | 无需重算，作为硬约束 |
| 平均开发强度 statutory_floor_area_intensity | 2408.0万㎡ ÷ 1645.6公顷 = 1.46 万㎡/公顷（推导基线） | 中 | 分母为城乡建设用地推导值 | 以官方图则用地分解复核 |
| 人口/岗位基线 population/employment_baseline | 读控规文本第 7 条常量 36.4 万 / 39.7 万 | 高（法定） | 无 | 无需重算 |
| 算法贡献指数 algorithm_contribution_index | 运营期：模型调用频次 × 场景权重 × 效果评估分 | 未知（运营期） | 依赖平台运行数据 | 数据契约上线后按季度校准 |
| 数据契约覆盖率 data_covenant_coverage_ratio | 已登记 L1/L2/L3 场景数 ÷ 场景总数（第 82 条法源） | 低（目标值 60%） | 场景清单逐年演进 | 每年随项目库更新重登记 |
| 自适应公共空间占比 adaptive_public_space_ratio | 可调公共空间面积 ÷ 公共空间总面积（目标 30%） | 低（目标值） | 依赖实施认定 | 实施进度后逐年复测 |

**验证流程**：所有「类别一（可直接复算）」指标由 `scripts/spatial_review.py` 与 `scripts/metrics.py` 输出正式自证证据；「类别二（法定常量）」指标以 `[source:REGULATORY-PLAN-HD00-1601]` 条款为唯一依据；「类别三（运营校准）」指标仅定义计算口径，正式上线后校准。任何一次官方边界发布、文保坐标复测、实测建筑基底替换都会触发对应指标的重新计算，并将新值与差异写入 `metrics.json` 变更记录，确保评审与归档阶段的指标均来自最近的确定口径。

### 2408 万㎡ 法定总量到空间结构的分摊测算（方案情景，非承诺值）

**口径声明**：下表为把控规第 7/13 条 2408.0 万㎡ 总量按「一带一轴，两心多点」（第 9 条）结构分配到九街区与三大重点片区的情景测算，用于论证方案情景在硬约束内的可行性；**不含任何地块级指标承诺、不含财务或投资数据**，地块最终指标以官方图则为准。

| 空间组成 | 情景分摊（万㎡） | 占比 | 测算逻辑 |
| --- | --- | --- | --- |
| 五道口中心（原点社区） | 420 | 17.4% | 折叠中枢：近校成果转化 + 人才社区，叠加轨道站点高强度复合 |
| 大钟寺中心 | 380 | 15.8% | 流通接口：站城一体化 + 数据资产交易载体 |
| 京张遗址公园产业创新带沿线 | 520 | 21.6% | 一带：低矮科创交往空间 + 更新载体（官方口径 100万㎡ 更新载体的放大辐射） |
| 学北园（统筹研究范围对象） | 300 | 12.5% | 算力与数据沙盒产业，仅作统筹研究承载，不占控规法定范围内强度 |
| 其余八街区存量更新 | 788 | 32.7% | 以存量改造为主，拆旧建新严格控制 |
| **合计** | **2408** | **100%** | 严格等于控规总量上限，方案任何情景不突破 |

**分配自检规则**：① 五道口 + 大钟寺两心合计 800 万㎡ ≤ 总量的 34%，符合「两心多点」结构逻辑；② 一带沿线叠加于既有街区更新容量内，不额外突破街区强度五级分区（第 14 条）与 6 类基准高度分区（第 16 条）；③ 学北园分摊计入统筹研究承载、不计入控规法定范围内强度，与 A-SCOPE-NESTED-001 口径一致。该分摊为设计论证参数，**不构成对官方审批的控制值承诺** `[assumption:A-FLOORAREA-001]`。

### 控规法定条款逐条核验表

下表将控规文本本方案实际引用的法定条款，按「条款原文要点 → 方案响应 → 验证方式 → 风险与回退」逐条核对。响应均落位到具体章节、图层或指标，确保任务响应性可机器复核 `[source:REGULATORY-PLAN-HD00-1601]`。

| 条款 | 控规要点 | JDSFB 响应 | 验证方式 | 风险与回退 |
| --- | --- | --- | --- | --- |
| 第 2 条 | 规划范围 16.7 km²、9 街区 | 三层范围框架中作为法定基准；与公告 11.4 km² 总体设计范围作嵌套关系（A-SCOPE-NESTED-001） | `site-overview` 图、metrics.total_floor 引用范围常量 | 官方正式边界未发布；回退：控规四至 + provisional 边界交叉校核 |
| 第 7 条 | 常住 36.4 万 / 就业 39.7 万 / 总建筑规模上限 2408.0 万㎡ | 人口、岗位、总量基线作为硬约束与情景测算基准 | [metric:population_baseline] [metric:employment_baseline] [metric:total_floor_area_ceiling_sqm] | 无（法定常量）；分摊测算仅为论证参数 |
...
| 第 13 条 | 总建筑规模上限 2408.0 万㎡（强约束） | 写入生成约束与自检（A-FLOORAREA-001），任何情景不突破 | `2408 万㎡ 分摊测算` 小节 | 突破风险（控制：硬编码到自检脚本） |
| 第 14 条 | 基准强度五级分区 | 更新强度不突破对应五级分区 | [metric:floor_area_ratio] 复核 | 图则未数字化，地块级强度结论标注「待正式图则」 |
| 第 16 条 | 36/45/60/80/100 米 6 类基准高度分区 | 建筑高度严守分区；1+3+N 边缘节点为临时模块不计入高度 | 高度分区在图则 | 地块级高度待图则确认 |
