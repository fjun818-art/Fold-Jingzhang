...
| --- | --- | --- | --- | --- |
| 禁止采集 | 无授权路段影像；儿童上下学路线个体轨迹 | 人脸/生物特征；个体级情绪数据；医疗推断 | 诊疗数据进入城市系统；无知情同意的健康监测 | 与贡献无关的个人敏感属性（性别、民族、户籍等） |
...
| 空间组成 | 情景分摊（万㎡） | 占法定范围内分摊 | 测算逻辑 |
| --- | --- | --- | --- |
| 五道口中心（原点社区） | 420 | 19.9% | 折叠中枢：近校成果转化 + 人才社区，叠加轨道站点高强度复合 |
| 大钟寺中心 | 380 | 18.0% | 流通接口：站城一体化 + 数据资产交易载体 |
| 京张遗址公园产业创新带沿线 | 520 | 24.7% | 一带：低矮科创交往空间 + 更新载体（官方口径 100万㎡ 更新载体的放大辐射） |
| 其余八街区存量更新 | 788 | 37.4% | 以存量改造为主，拆旧建新严格控制 |
| **法定范围内合计** | **2108** | **100%** | ≤ 2408 万㎡ 法定上限，方案任何情景不突破 |
...
**分配自检规则**：① 法定范围内合计 2108 万㎡ ≤ 2408 万㎡ 法定上限（余量 300 万㎡ 作为弹性缓冲，待官方图则细化后分配）；② 五道口 + 大钟寺两心合计 800 万㎡，占法定范围内分摊的 38.0%，符合「两心多点」结构逻辑；③ 一带沿线叠加于既有街区更新容量内，不额外突破街区强度五级分区（第 14 条）与 6 类基准高度分区（第 16 条）；④ 学北园 300 万㎡ 位于 16.7 km² 法定范围之外，属统筹研究范围（43.6 km²）承载，单列核算、不参与 2408 万㎡ 法定总量的分配与合计，与 A-SCOPE-NESTED-001 口径一致。该分摊为设计论证参数，**不构成对官方审批的控制值承诺** `[assumption:A-FLOORAREA-001]`。
