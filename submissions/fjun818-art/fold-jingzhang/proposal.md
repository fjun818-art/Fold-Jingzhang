...
- **来源登记状态声明**：截至本版本提交日，data/source_registry.json 尚无本方案所引外部材料的 approved_formal 记录；`sources.json` 中标注 official_public 的条目均为参与方基于公开渠道自行获取并提交，其可溯达性以 `sources.json` 登记的 URL、发布机构、文号/版本与获取日期为准，待组织方来源登记复核后方可作为正式评分依据。正文中「法定」「官方」等表述指文件自身的法律/发布属性，不代表本方案已完成正式核验登记。
...
控规第 82 条明确「探索数据要素市场化配置改革创新路径」[source:REGULATORY-PLAN-HD00-1601]。京张数智折叠带 将该法定条款实施性深化为「京张数据契约」机制：
...
本节给出 京张数智折叠带 场景设计与国家城市全域数字化转型、城市数字更新最新行业指引的**一致性验证结果**：经逐项比对，10 个 AI+ 运营环节在场景分类、总体原则、数字底座、场景结构与运营机制五个维度均与国家行业指引保持一致 [source:DIGITAL-RENEWAL-GUIDELINE]。
...
**验证结论**：京张数智折叠带 场景体系与国家城市数字更新行业指引方向一致、结构兼容、机制可对接，可作为指引在 AI 创新街区场景下的概念性落地参考。所有 AI 应用同时遵循本方案「伦理与数据治理」边界。
...
**证据等级说明（来源待复核期间的临时口径）**：在 `data/source_registry.json` 完成正式登记前，本方案引用的控规、三区两翼发布、数字更新指引等外部文件均属**参与方提交、needs-review/待复核证据**，并非已登记的 approved_formal 正式证据。因此下表与图件中标注为「法定」「高置信」的项目，其置信度仅指**参与方就公开获取文本的可读引用而言的内部校核置信度**，不表示该来源已获组织方来源登记审核通过；图 1、图 5 及正文中的相关置信度分级亦按此临时口径理解，登记审核完成后统一以登记状态为准。
...
| 容积率法定基线 [metric:statutory_floor_area_intensity] | 14632 ㎡/ha（约 1.463 万㎡/ha） | 二 | 控规文本第 7 条 |
...
**口径声明**：下表仅针对控规法定范围（16.7 km²，九街区）分配控规第 7/13 条 2408.0 万㎡ 总量，按「一带一轴，两心多点」（第 9 条）结构落到街区与重点片区的情景测算，用于论证方案情景在硬约束内的可行性；**不含任何地块级指标承诺、不含财务或投资数据**，地块最终指标以官方图则为准。

**分区互斥声明**：下表四个分区**互斥、不重叠**——按主导功能在空间上互不重叠地切分 16.7 km² 法定范围，任何建设容量只计入一个分区，不重复计量。「京张遗址公园产业创新带」**仅统计遗址公园本体及其两侧紧邻的更新改造带**（不再与两心空间范围叠加）；两心范围内的遗址公园路段容量已含在对应「两心」行内，不在「一带」行重复计入。

**法定范围内（16.7 km²）互斥分摊：**

| 互斥空间分区 | 情景分摊（万㎡） | 占法定范围内分摊 | 分区边界与测算逻辑 |
| --- | --- | --- | --- |
| 五道口中心（原点社区） | 420 | 19.9% | 折叠中枢：近校成果转化 + 人才社区，叠加轨道站点高强度复合；**含其范围内遗址公园路段容量** |
| 大钟寺中心 | 380 | 18.0% | 流通接口：站城一体化 + 数据资产交易载体；**含其范围内遗址公园路段容量** |
| 遗址公园沿线更新带（两心以外） | 520 | 24.7% | 遗址公园本体及两侧紧邻更新带的低矮科创交往空间（官方口径 100万㎡ 更新载体落位带）；**与两心不重叠** |
| 其余街区存量更新（两心与一带以外） | 788 | 37.4% | 两心及遗址公园沿线以外的其余街区，以存量改造为主，拆旧建新严格控制 |
| **法定范围内合计** | **2108** | **100%** | 各分区面积互斥合计；2108 ≤ 2408 万㎡ 法定上限 |

**不重复计量校核**：四分区按主导功能在空间上互斥，任一容量只计入一个分区；五道口 + 大钟寺两心合计 800 万㎡（38.0%），遗址公园沿线更新带 520 万㎡（24.7%），其余街区 788 万㎡（37.4%），合计 2108 万㎡，无重复计入。

**法定范围外单列（不计入 2408 万㎡ 法定总量）：**

| 空间组成 | 情景规模（万㎡） | 口径 |
| --- | --- | --- |
| 学北园（统筹研究范围 43.6 km² 内、16.7 km² 法定范围外） | 300 | 统筹研究承载：算力与数据沙盒产业，**单列核算，不占用、不计入控规法定总量** |

**分配自检规则**：① 法定范围内四分区互斥合计 2108 万㎡ ≤ 2408 万㎡ 法定上限（余量 300 万㎡ 作为弹性缓冲，待官方图则细化后分配）；② 五道口 + 大钟寺两心合计 800 万㎡，占法定范围内分摊的 38.0%，符合「两心多点」结构逻辑；③ 各分区分别受对应街区强度五级分区（第 14 条）与 6 类基准高度分区（第 16 条）约束，遗址公园沿线带的低矮科创空间不突破其所在分区强度上限；④ 学北园 300 万㎡ 位于 16.7 km² 法定范围之外，属统筹研究范围（43.6 km²）承载，单列核算、不参与 2408 万㎡ 法定总量的分配与合计，与 A-SCOPE-NESTED-001 口径一致。该分摊为设计论证参数，**不构成对官方审批的控制值承诺** `[assumption:A-FLOORAREA-001]`。

### 控规法定条款逐条核验表

下表将控规文本本方案实际引用的法定条款，按「条款原文要点 → 方案响应 → 验证方式 → 风险与回退」逐条核对。响应均落位到具体章节、图层或指标，确保任务响应性可机器复核 `[source:REGULATORY-PLAN-HD00-1601]`。

| 条款 | 控规要点 | 京张数智折叠带 响应 | 验证方式 | 风险与回退 |
| --- | --- | --- | --- | --- |
