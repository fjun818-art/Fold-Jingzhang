...
summary: "京张数智折叠带：以控规为基准（参与方提交·待来源登记复核）、以「数据要素折叠」为核心机制的 AI 城市设计方案。7 个时空折叠点全部锚定控规文保清单，京张数据契约对接控规第 82 条数据要素市场化条款，建筑规模严守 2408.0 万㎡ 总量上限；方案在空间布局、历史文化保护、基础设施更新、生态环境治理、产业转型升级五大重点领域与《城市更新「十五五」规划》对齐，并按「专项规划—片区策划—项目实施方案」体系给出实施路径与政策资金监督机制；使用 provisional boundary 并保留精度警示，组织方数据缺口不阻断内容评分。"
...
### 2408 万㎡ 总量到空间结构的分摊测算（方案情景，非承诺值）

**口径声明**：下表仅针对控规范围（16.7 km²，九街区，待来源登记复核）分配控规第 7/13 条 2408.0 万㎡ 总量，按「一带一轴，两心多点」（第 9 条）结构落到街区与重点片区的情景测算，用于论证方案情景在硬约束内的可行性；**不含任何地块级指标承诺、不含财务或投资数据**，地块最终指标以官方图则为准。**完整可复算口径（公式、四分区 breakdown、约束条件、触发重算规则）见 `metrics.json → floor_area_scenario_allocation`**。
...
| 互斥空间分区 | 情景分摊（万㎡） | 占控规范围内分摊 | 分区边界与测算逻辑 |
| --- | --- | --- | --- |
...
| **控规范围内合计** | **2108** | **100%** | 各分区面积互斥合计；2108 ≤ 2408 万㎡ 总量上限（待复核） |
...
**分配自检规则**：① 控规范围内四分区互斥合计 2108 万㎡ ≤ 2408 万㎡ 总量上限（待复核，余量 300 万㎡ 作为弹性缓冲，待官方图则细化后分配）；② 五道口 + 大钟寺两心合计 800 万㎡，占控规范围内分摊的 38.0%，符合「两心多点」结构逻辑；③ 各分区分别受对应街区强度五级分区（第 14 条）与 6 类基准高度分区（第 16 条）约束，遗址公园沿线带的低矮科创空间不突破其所在分区强度上限；④ 学北园 300 万㎡ 位于 16.7 km² 控规范围之外，属统筹研究范围（43.6 km²）承载，单列核算、不参与 2408 万㎡ 总量的分配与合计，与 A-SCOPE-NESTED-001 口径一致。该分摊为设计论证参数，**不构成对官方审批的控制值承诺** `[assumption:A-FLOORAREA-001]`。
...
| 第 7 条 | 常住 36.4 万 / 就业 39.7 万 / 总建筑规模上限 2408.0 万㎡ | 人口、岗位、总量基线作为硬约束与情景测算基准 | [metric:population_baseline] [metric:employment_baseline] [metric:total_floor_area_ceiling_sqm] | 来源待登记复核；分摊测算仅为论证参数 |
