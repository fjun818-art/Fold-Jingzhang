...
视频演示按「数据流转视频」与「AI 赋能视频」两类组织。成片规格统一为：MP4 封装（H.264 编码），分辨率不低于 1920×1080（1080p），帧率不低于 30fps，音频 AAC 编码（采样率 44.1kHz，比特率不低于 128kbps），单个视频文件不超过 200MB。每条视频配标准化文字说明（唯一标识符、精确时长、核心内容概述、关键时间节点标记），说明文字放置于视频播放器下方。视频素材正在制作中，到位后登记 manifest.json 并替换本节占位说明。
...
- **关键时间节点标记**：00:00 片头与折叠概念引入；折叠点 AR/MR 叙事演示；AI+ 运营场景串联；结尾（精确时点待剪辑后标注）。
